...
def fetch_ff_factors_monthly(
    to_path: str = "datasets/ken_french/ff_factors_monthly.csv",
):
    ...
    start_idx = next(
        i for i, line in enumerate(content) if len(line) >= 6 and line[:6].isdigit()
    )
    end_idx = next(
        i
        for i in range(start_idx, len(content))
        if content[i].lower().startswith("annual factors")
    )
